...
// (snake_case -> Title Case, "_px" suffix -> "(px)", "_deg" suffix -> "(°)")
// kept in sync on both sides so the PDF and the interactive report show
// identical metric labels for the same raw measurement.metrics keys.
...
export function humanizeMetricKey(key: string): string {
  ...
  if (key.endsWith("_deg")) return `${title(key.slice(0, -"_deg".length))} (°)`
  return title(key)
}
